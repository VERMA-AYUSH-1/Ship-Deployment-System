...
def plotMap(data, opsName, opsColour):
    ...
    for _, ap in data.iterrows():
        folium.Marker(
            ...
            popup=(ap["Name "], ap["Pennant No."], ap["Country"],
                   ap["Type"], ap["CLASS"], ap["Speed"], ap["Length"]),
            tooltip=(ap["Name "], ap["Pennant No."], ap["Country"],
                     ap["Type"], ap["CLASS"], ap["Speed"], ap["Length"]),
...
def plotCmdMap(data, opsName, opsColour):
    ...
    # ship['colors'] = ship.apply(select_marker_color, axis=1)
    # ship.head(3)
